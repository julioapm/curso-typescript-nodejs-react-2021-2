...
export class MoedaRepositorio {
    ...
    static async buscaPorCodigo(codigo:string): Promise<Moeda|null> {
        return MoedaModel.findOne({codigo: codigo}).exec();
        //return MoedaModel.where('codigo').equals(codigo).findOne().exec();
    }
}

/*export async function criar(moeda: Moeda): Promise<Moeda> {
        return MoedaModel.create(moeda);
}*/
